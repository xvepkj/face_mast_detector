...
Dict = {}
lastname = "unknown"
def detect_and_predict_face(frame, detector, embedder,recognizer):
	...
	for i in range(0, detections.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			global lastname
			global Dict
			if(lastname!=name and name!="unknown"):
				engine = pyttsx3.init()
				engine.say(name+"please wear your mask")
				engine.runAndWait()
...
ap.add_argument("-m", "--embedding-model", type = str,
...
while True:
	...
	# dimensions
	detect_and_predict_face(frame,detector,embedder,recognizer)
